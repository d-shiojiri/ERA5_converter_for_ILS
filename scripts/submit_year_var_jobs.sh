...
submit_config="${ERA5_SUBMIT_CONFIG:-$REPO_ROOT/configs/submit_default.yaml}"
...
usage() {
  cat <<USAGE
Usage:
  submit_year_var_jobs.sh --stage <prep|convert> --years <LIST|RANGE> [options]
...
Config files (env override):
  ...
  ERA5_SUBMIT_CONFIG=$submit_config
USAGE
}
...
if [[ -z "$stage" || -z "$years_arg" ]]; then
  echo "Missing required args: --stage and --years" >&2
  usage
  exit 2
fi
...
if [[ "$executor" == "sbatch" ]] && is_truthy "$auto_adjust_cpus_for_mem" && [[ -n "$mem" ]]; then
  ...
  if req_mem_mib="$(mem_to_mib "$mem")"; then
    ...
    if (( cpus < min_cpus_for_mem )); then
      echo "[submit] cpus=$cpus is too small for mem=$mem under ${assumed_mem_per_cpu_gb}G/cpu policy; auto-adjusting cpus to $min_cpus_for_mem"
      cpus="$min_cpus_for_mem"
    fi
  else
    echo "[submit] warning: failed to parse mem value '$mem'; skipping cpu auto-adjust" >&2
  fi
fi
...
echo "[submit] stage=$stage executor=$executor years=${#years[@]} vars=${#vars[@]} max_parallel_jobs=$max_parallel_jobs cpus=$cpus mem=${mem:-auto}"
...
for year in "${years[@]}"; do
  for var_name in "${vars[@]}"; do
    job_name="era5_${stage}_${year}_${var_name}"
    ...
    if [[ "$executor" == "sbatch" ]]; then
      ...
      if [[ "$dry_run" -eq 1 ]]; then
        ...
        job_id="${submit_out%%;*}"
        if ! [[ "$job_id" =~ ^[0-9]+$ ]]; then
          echo "[submit] failed to parse sbatch job id: $submit_out" >&2
          exit 2
        fi
